...
class LanceMapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item_dict["intensity_array"] = np.array(item_dict["intensity_array"], dtype=np.float32)
        ...
        if self.seq_len is not None and len(item_dict["mz_array"]) < self.seq_len:
            ...
            item_dict["intensity_array"] = np.pad(item_dict["intensity_array"], (0, pad_right))
    ...
    def __init__(self, run_dfs, run_labels=None, seq_len=None):
        ...
        for run_df in tqdm(run_dfs):
            ...
            if self.seq_len is not None:
                ...
                run_intensity_arrays = [self._pad_sequence(seq) for seq in run_intensity_arrays]
                run_intensity_arrays = np.stack(run_intensity_arrays, axis=0).astype(np.float32)
