...
def load_scalars(run_dir: str) -> dict[str, tuple[list[int], list[float]]]:
    ...
    merged: dict[str, tuple[list[int], list[float]]] = defaultdict(
        lambda: ([], [])
    )
...
def main():
    ...
    for ax, (panel_name, wanted) in zip(axes, PANELS):
        ...
        for ri, (run_name, scalars) in enumerate(runs.items()):
            ...
            for tag, (steps, vals) in scalars.items():
                ...
                if panel_name == "loss" and "loss_mz_bin" in low:
                    continue
                # solid for train, dashed for val, to tell the pair apart per run
                style = "--" if "val" in low else "-"
                lbl = tag if len(runs) == 1 else f"{run_name}:{tag}"
                ax.plot(
                    steps, vals, style, color=color, linewidth=1.5, alpha=0.9, label=lbl
                )
                plotted = True
        ...
        if plotted:
            ...
            ax.text(0.5, 0.5, f"no '{panel_name}' scalars", ha="center", va="center",
